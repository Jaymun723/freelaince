...
class OffersManager {
  ...
  handleServerMessage(data) {
    ...
    switch (data.type) {
      ...
      case 'offer_status_updated':
        if (data.success) {
          this.showSuccess(`Offer status updated to ${data.status}`);
          // Refresh offers to get latest data
          this.requestOffers();
        } else {
          this.showError('Failed to update offer status: ' + (data.error || 'Unknown error'));
        }
        break;
      case 'error':
  ...
  requestOffers() {
    if (this.wsSocket && this.wsSocket.readyState === WebSocket.OPEN) {
      console.log('📡 Requesting offers from server...');
  ...
  setupOfferActionListeners() {
    // Status update buttons
    document.querySelectorAll('.status-btn').forEach(button => {
      button.addEventListener('click', (e) => {
        const offerId = e.target.dataset.offerId;
        const newStatus = e.target.dataset.status;
        this.updateOfferStatus(offerId, newStatus);
      });
    });
  ...
  renderOffers() {
    ...
    this.setupOfferActionListeners();
  }
    ...
    return `
      <div class="offer-card ${offer.status}" data-offer-id="${offer.offer_id}">
        ...
        <div class="offer-actions">
          ${offer.source_url ? `<button class="btn btn-outline btn-small view-source-btn" data-url="${offer.source_url}">View Source</button>` : ''}
          ${offer.status === 'pending' ? `
            <button class="btn btn-outline btn-small status-btn" data-offer-id="${offer.offer_id}" data-status="accepted">Accept</button>
            <button class="btn btn-outline btn-small status-btn" data-offer-id="${offer.offer_id}" data-status="declined">Decline</button>
          ` : ''}
          ${offer.status === 'accepted' ? `
            <button class="btn btn-outline btn-small status-btn" data-offer-id="${offer.offer_id}" data-status="completed">Mark Complete</button>
            <button class="btn btn-outline btn-small status-btn" data-offer-id="${offer.offer_id}" data-status="declined">Cancel</button>
          ` : ''}
          ${offer.status === 'declined' ? `
            <button class="btn btn-outline btn-small status-btn" data-offer-id="${offer.offer_id}" data-status="pending">Reconsider</button>
          ` : ''}
          ${offer.status === 'completed' ? `
            <button class="btn btn-outline btn-small status-btn" data-offer-id="${offer.offer_id}" data-status="accepted">Reopen</button>
          ` : ''}
          <button class="btn btn-primary btn-small details-btn" data-offer-id="${offer.offer_id}">Details</button>
        </div>
      </div>
    `;
  }
    ...
    try {
      ...
      if (this.wsSocket && this.wsSocket.readyState === WebSocket.OPEN) {
        ...
        if (offer) {
          ...
          this.applyFilters();
          this.updateStats();
          this.showSuccess(`Offer status updated to ${newStatus} (offline)`);
        }
      }
